...
public class TimeUtil {
    ...
    private static String YESTERDAY = XWCodeTrans.doTrans("昨天");
    private static String BEFORE_YESTERDAY = XWCodeTrans.doTrans("前天");
    ...
    private static String getTimeTips(long timeStamp) {
        ...
        if (diff < 60) {
            tips = xechwic.android.XWCodeTrans.doTrans("刚刚");
        } else if ((diff /= 60) < 60) {
            tips = String.format("%d"+xechwic.android.XWCodeTrans.doTrans("分钟前"), diff);
        } else if ((diff /= 60) < 24) {
            tips = String.format("%d"+xechwic.android.XWCodeTrans.doTrans("小时前"), diff);
        } else if ((diff /= 24) < 7) {
            tips = String.format("%d"+xechwic.android.XWCodeTrans.doTrans("天前"), diff);
        } else if ((diff /= 7) < 4) {
            tips = String.format("%d"+xechwic.android.XWCodeTrans.doTrans("周前"), diff);
        } else {
