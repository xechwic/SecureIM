package xechwic.android.view.expand;
...
public class IphoneTreeView extends ExpandableListView implements
	...
	/**
	 * 
	 * 点击了 GroupBean 触发的事件 , 要根据根据当前点击 GroupBean 的状态来
	 */
	@Override
	public boolean onGroupClick(ExpandableListView parent, View v,
			int groupPosition, long id) {
		if (mAdapter.getHeadViewClickStatus(groupPosition) == 0) {
			collapseGroupAll(parent);
			mAdapter.onHeadViewClick(groupPosition, 1);
	...
	private void collapseGroupAll(ExpandableListView parent){
		if(mAdapter instanceof ExpandableListAdapter){
			int index=((ExpandableListAdapter)mAdapter).getGroupCount();
			if(index>0){
